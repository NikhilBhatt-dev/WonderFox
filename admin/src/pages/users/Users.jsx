    ...
    return (
        <AdminLayout>
            <div className="mb-6 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 rounded-lg bg-blue-50 px-4 py-2 font-semibold text-blue-700">
                    ...
                    <tbody>
                        {loading ? <tr><td colSpan={8} className="py-12 text-center text-gray-500">Loading users...</td></tr> : error ? (
                            ...
                            <tr key={user._id} className="border-t text-sm hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4"><span className={`rounded-full px-3 py-1 text-xs font-semibold ${user.isActive ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>{user.isActive ? "Active" : "Inactive"}</span></td>
                                <td className="px-6 py-4 text-center"><Link to={`/users/${user._id}`} className="inline-flex items-center gap-1.5 text-blue-600 hover:underline"><Eye size={16} /> View</Link></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
